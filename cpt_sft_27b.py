...
def main():
    ...
    outputDir = f"models/output_gemma3_27b_cpt_{now}_{format_lr(lr)}_b{train_batch_size*accumulation_steps}x{gpu_num}_{train_epochs}epo"
    runName = f"gemma3_27b_cpt_{now}_{format_lr(lr)}_b{train_batch_size*accumulation_steps}x{gpu_num}_{train_epochs}epo"
    ...
    # trainer.save_model(f"models/final_gemma3_27b_cpt_{now}")
    # tokenizer.save_pretrained(f"models/final_gemma3_27b_cpt_{now}")
    print("CPT completed!")
    ...
    outputDir = f"models/output_gemma3_27b_sft_{now}_{format_lr(lr)}_b{train_batch_size*accumulation_steps}x{gpu_num}_cosine_{train_epochs}epo_gap"
    runName = f"gemma3_27b_sft_{now}_{format_lr(lr)}_b{train_batch_size*accumulation_steps}x{gpu_num}_cosine_{train_epochs}epo_gap"
    ...
    print("Starting SFT...")
    trainer_stats = trainer.train()
    # trainer.save_model(f"models/final_gemma3_27b_sft_{now}")
    # tokenizer.save_pretrained(f"models/final_gemma3_27b_sft_{now}")
    print("SFT completed!")
